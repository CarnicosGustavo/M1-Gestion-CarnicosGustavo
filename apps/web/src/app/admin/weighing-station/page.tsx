...
export default function WeighingStationPage() {
	...
	// Resetear índice al cambiar de orden (el peso lo precarga el efecto de abajo
	// según el artículo actual).
	useEffect(() => {
		setCurrentItemIndex(0);
	}, [selectedOrderId]);
	...
	// Al cambiar de artículo: precargar el peso solicitado (el kg capturado al crear
	// el pedido) como referencia y dejar el cursor en el input. El operador ajusta
	// al peso real de la báscula. Así el dato capturado NO se "pierde".
	const weightInputRef = useRef<HTMLInputElement>(null);
	// biome-ignore lint/correctness/useExhaustiveDependencies: solo al cambiar de ítem
	useEffect(() => {
		const reqKg = currentItem?.quantity_kg
			? Number(currentItem.quantity_kg).toString()
			: "";
		setActualWeight(reqKg);
									...
									<button
										...
										className={cn(
											...
											newOrderIds.has(order.id) &&
												"animate-pulse bg-green-100 ring-2 ring-green-400 ring-inset",
										)}
									>
							...
							<CardContent className="flex flex-1 flex-col gap-5 overflow-y-auto p-6">
								...
								<div className="space-y-1 text-center">
									...
									<h2 className="font-extrabold text-4xl tracking-tight">
										{currentItem.quantity_pieces}&nbsp;
										{currentItem.product?.name ?? currentItem.product_name}
									</h2>
									...
									{currentItem.quantity_kg ? (
										<p className="font-semibold text-primary text-sm">
											Solicitado: {Number(currentItem.quantity_kg).toFixed(3)}{" "}
											kg
											<span className="ml-1 font-normal text-muted-foreground">
												(referencia · ajusta al peso real)
											</span>
										</p>
									) : null}
								</div>
									...
									<div className="flex items-center justify-between gap-2">
										...
										<div className="flex gap-1">
											...
											<button
												...
												disabled={
													!actualWeight || Number.parseFloat(actualWeight) <= 0
												}
												className="rounded-lg border border-blue-300 bg-blue-50 px-3 py-1 font-bold text-blue-700 text-xs uppercase tracking-wider transition-colors hover:bg-blue-100 disabled:cursor-not-allowed disabled:opacity-40"
			...
			{completedOrder && (
				<div className="fade-in slide-in-from-bottom-4 fixed right-6 bottom-6 z-50 animate-in">
					<Link href={`/admin/checkout?order=${completedOrder.id}`}>
						<Button
							size="lg"
							className="h-14 rounded-2xl bg-green-600 px-6 font-bold text-base shadow-xl hover:bg-green-700"
